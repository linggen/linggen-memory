...
2. One Bash call combining, all failure-tolerant (`|| true`, `--max-time 2`):
   ...
   - `curl -s --max-time 2 http://127.0.0.1:9527/api/bridge/status` — is the browser extension connected (`connected`)
...
```
linggen: ling-mem <v> · engine <v> · browser ok
...
- ling-mem daemon down → first line says `ling-mem down — ling-mem start`; skip memory/upkeep lines. Engine down → `engine —`. Browser extension not connected → `browser —`.
